...
void MainWindow::dibujarNeurona() {
    ...
    // Asegúrate de que la escena se haya creado previamente en el ámbito principal
    ...
    for (int i = 0; i < neuronasCount; i++) {
        ...
        int x = neurona->getPX();
        int y = neurona->getPY();
        float voltaje = neurona->getaVolt();
        int red = neurona->getRed();
        int green = neurona->getGreen();
        int blue = neurona->getBlue();

        QPointF topLeft(x, y);            // Coordenadas del punto superior izquierdo
        QPointF bottomRight(x + 10, y + 10); // Coordenadas del punto inferior derecho
        QRectF ellipseRect(topLeft, bottomRight);  // Crear el rectángulo

        // Crea un QGraphicsEllipseItem para representar la partícula (neurona)
        QGraphicsEllipseItem *particleItem = new QGraphicsEllipseItem(ellipseRect);
        particleItem->setPos(x, y);

        // Establece el color según el valor de los componentes RGB
        particleItem->setBrush(QColor(red, green, blue));

        // Agrega el elemento a la escena
